            ...
            <div class="p-6">
                <form method="POST" action="{{ route('register.custom') }}">
                    ...
                    <div class="grid lg:grid-cols-2 gap-6 mb-6">
                        ...
                        <div>
                            <label class="block text-sm font-medium text-default-900 mb-2 dark:text-gray-200" for="cellphone">Telefono</label>
                            <input id="cellphone" required autocomplete="phone" class="dark:text-gray-200 dark:border-gray-700 block w-full bg-transparent rounded-full py-2.5 px-4 border border-default-200 focus:ring-transparent focus:border-default-200 dark:bg-default-50 form-control @error('phone') is-invalid @enderror" name="cellphone" value="{{ old('phone') }}" type="tel" placeholder="3113112232">
                            @error('cellphone')
                                <span class="invalid-feedback text-red-600" role="alert">
                                    <strong>{{ $message }}</strong>
                                </span>
